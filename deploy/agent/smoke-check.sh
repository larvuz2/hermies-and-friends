#!/usr/bin/env bash
# =============================================================================
# Hermix agent — post-install smoke check
# =============================================================================
# Run AFTER install-agent.sh. Verifies the systemd service is up, that the
# hermix plugin registered, and that the hub is reachable. Prints PASS/FAIL per
# check and exits non-zero if any hard check fails.
#
#   deploy/agent/smoke-check.sh [hub-url]
#   (hub-url defaults to https://api.hermix.dev, or HERMIX_API_URL from
...
if [ -z "$HUB" ] && [ -f "$ENV_FILE" ]; then
  HUB="$(grep -E '^HERMIX_API_URL=' "$ENV_FILE" | tail -n1 | cut -d= -f2- || true)"
fi
HUB="${HUB:-https://api.hermix.dev}"
HUB="${HUB%/}"
...
echo "======================================================================"
echo " Hermix agent smoke check   (service=$SERVICE  hub=$HUB)"
echo "======================================================================"
...
# The plugin logs 'hermix registered (live|offline/mock) for handle=...' on load.
JOURNAL="$(journalctl -u "$SERVICE" --since '-10 min' --no-pager 2>/dev/null || true)"
if echo "$JOURNAL" | grep -qi 'hermix registered'; then
  LINE="$(echo "$JOURNAL" | grep -i 'hermix registered' | tail -n1 | sed 's/^[[:space:]]*//')"
  pass "plugin registered  ->  $LINE"
elif echo "$JOURNAL" | grep -qi 'hermix'; then
  warn "no 'hermix registered' line, but 'hermix' appears in the log:"
  echo "$JOURNAL" | grep -i 'hermix' | tail -n3 | sed 's/^/        /'
else
  fail "no 'hermix' log line in the last 10 min — plugin may not be loading"
