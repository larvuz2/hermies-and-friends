...
from hermies import _config, profile, commands
from hermies.client import (HermiesClient, HttpTransport, ensure_registered,
                            make_transport)
...
def test_default_hub_is_the_real_hub(monkeypatch):
    ...
    assert _config.service_url() == "https://srv1691895.hstgr.cloud"
...
def test_persist_api_key_sets_process_and_file(monkeypatch, tmp_path):
    envf = tmp_path / ".env"
    monkeypatch.setenv("HERMIES_ENV_FILE", str(envf))
    monkeypatch.delenv("HERMIES_API_KEY", raising=False)
    _config.persist_api_key("sk-live-123")
    assert os.environ["HERMIES_API_KEY"] == "sk-live-123"
    assert "HERMIES_API_KEY=sk-live-123" in envf.read_text(encoding="utf-8")


# --- ensure_registered ------------------------------------------------------
def test_ensure_registered_noop_when_already_keyed(monkeypatch):
    monkeypatch.setenv("HERMIES_API_KEY", "already")
    assert ensure_registered(HermiesClient(MockBackend()),
...
def test_ensure_registered_claims_and_persists_key(monkeypatch, tmp_path):
    monkeypatch.delenv("HERMIES_API_URL", raising=False)      # default real hub
    monkeypatch.delenv("HERMIES_API_KEY", raising=False)
    monkeypatch.setenv("HERMIES_ENV_FILE", str(tmp_path / ".env"))
    ok = ensure_registered(HermiesClient(MockBackend()),      # mock -> "mock-key"
...
def test_publish_auto_registers(monkeypatch, tmp_path):
    ...
    monkeypatch.delenv("HERMIES_API_KEY", raising=False)
    monkeypatch.setenv("HERMIES_ENV_FILE", str(tmp_path / ".env"))
    handler = commands.make_handler(HermiesClient(MockBackend()),
...
# --- connectivity in /hermies status ---------------------------------------
def test_status_offline_banner(monkeypatch, tmp_path):
    monkeypatch.setenv("HERMIES_HOME", str(tmp_path))
    monkeypatch.setenv("HERMIES_API_URL", "")
    out = commands.make_handler(HermiesClient(MockBackend()),
...
def test_status_connected_banner(monkeypatch, tmp_path):
    monkeypatch.setenv("HERMIES_HOME", str(tmp_path))
    monkeypatch.delenv("HERMIES_API_URL", raising=False)
    monkeypatch.setenv("HERMIES_API_KEY", "key123")
    out = commands.make_handler(HermiesClient(MockBackend()),   # healthz -> True
