...
# --------------------------------------------------------------------------- #
# Budget exhaustion -> queue -> delivered next day
# --------------------------------------------------------------------------- #
def test_budget_exhaustion_queues_and_delivers_next_day(monkeypatch):
    ...
    matchmaker.run_cycle(state, client, card, llm, clock)      # two handshakes
    clock.advance(2 * DAY)
    client.push_reply("a-herald", "keen to collaborate")
    client.push_reply("b-herald", "keen to collaborate")
    out = matchmaker.run_cycle(state, client, card, llm, clock)  # both notify, budget=1

    assert out != matchmaker.SILENT
    assert len(state["queue"]) == 1                     # one overflow queued
    assert len(state["notify_log"]) == 1

    # Same day: budget still spent, overflow stays queued.
    clock.advance(5 * 3600)
    assert matchmaker.run_cycle(state, client, card, llm, clock) == matchmaker.SILENT
    assert len(state["queue"]) == 1

    # Next day: 24h window frees the slot, queued item delivered.
    clock.advance(DAY)
    out2 = matchmaker.run_cycle(state, client, card, llm, clock)
    assert out2 != matchmaker.SILENT
    assert state["queue"] == []


def test_two_notifies_batch_into_one_digest(monkeypatch):
    monkeypatch.setenv("HERMIES_MAX_NOTIFY_PER_DAY", "2")
    state = matchmaker.new_state()
    client = FakeClient([_sig("a-herald", "offers X", 5), _sig("b-herald", "offers Y", 5)])
    clock = Clock()
    card, llm = _card(), FakeLlm()
...
def test_no_reply_judged_on_cards_after_timeout(monkeypatch):
    ...
    out = matchmaker.run_cycle(state, client, card, llm, clock)
    assert out != matchmaker.SILENT                          # judged on cards alone
    assert state["seen"]["mira-herald"]["verdict"] == "notify"
